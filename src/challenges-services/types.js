/**
 * @fileoverview JSDoc typedef declarations for all challenge/game services.
 *
 ...
 *   @type {import('./types').GameWordResult}
 */
...
// ---------------------------------------------------------------------------
// Game / Challenge Schema Types
// ---------------------------------------------------------------------------

/**
 * Fixed category used as the third segment of a gameWords document ID.
 * Document ID pattern: `{game}__{learningDialect}__{category}`
 *
 * @typedef {'general'|'food'|'travel'|'sports'|'tech'|'nature'} GameWordCategory
 */

/**
 * Supported challenge/game identifiers.
 * Extend this union as new games are introduced.
 *
 * @typedef {'hangman'} GameId
 */

/**
 * Top-level document in the `gameWords` collection.
 * Path: gameWords/{game}__{learningDialect}__{category}
 *
 * @typedef {Object} GameWordPool
 * @property {GameId} game
 * @property {string} learningDialect - BCP-47 tag, e.g. 'es-MX'
 * @property {GameWordCategory} category
 ...
 * requested userDialect is absent, HangmanService generates it via AI
 ...
 * Tracks which word IDs this user has already seen.
 *
 * @typedef {Object} UserGameProgress
 * @property {string[]} seenWordIds - Array of GameWord document IDs
...
/**
 * Input parameters shared by all word-based game services.
 *
 * @typedef {Object} BaseGameServiceParams
 * @property {string} uid
 ...
 * Extends BaseGameServiceParams — add hangman-specific fields here if needed.
 *
 * @typedef {BaseGameServiceParams} HangmanServiceParams
 */
