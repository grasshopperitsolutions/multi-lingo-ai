...
const ExerciseSidebar = ({
  ...
  const hasExamScores = examSession?.finalScores != null;
  const examScores = examSession?.finalScores;
  ...
  const countAnswered = (sectionExercises) =>
    (sectionExercises ?? []).filter((ex) => ex.answers && Object.keys(ex.answers).length > 0).length;
  ...
  // Exam section navigation (only shown in exam mode during active phases)
  const examSectionNav = examMode && ["listening", "reading", "writing"].includes(phase) && (
  ...
  // Exam score panel (only shown in exam mode results phase)
  const examScorePanel = examMode && hasExamScores && examScores && (
    <div className={`${panelBase} p-4 flex flex-col gap-3`}>
      ...
      <p className={`text-4xl font-black tabular-nums leading-none ${
        ...
        {examScores.total}
        <span className={`text-lg font-bold ${labelClass}`}>/{examScores.totalMax}</span>
      </p>
      <p className={`text-xs font-bold ${labelClass}`}>
        {examScores.totalMax > 0 ? Math.round((examScores.total / examScores.totalMax) * 100) : 0}%
      </p>
      <div className={`pt-2 border-t-2 ${isDarkMode ? "border-slate-700" : "border-slate-200"} flex flex-col gap-1.5`}>
        ...
        <div className="flex justify-between text-xs font-bold">
          ...
          <span className={labelClass}>{examScores.writing}/{examScores.writingMax}</span>
        </div>
      </div>
    </div>
  );
