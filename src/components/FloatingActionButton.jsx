...
const ArrowLight = () => (
  <>
    {/* Border layer */}
    <span className="absolute top-full left-1/2 -translate-x-1/2 border-[5px] border-transparent border-t-slate-900" aria-hidden="true" />
    {/* Fill layer (sits 1px above, covers border with white) */}
    <span className="absolute top-[calc(100%-1px)] left-1/2 -translate-x-1/2 border-4 border-transparent border-t-white" aria-hidden="true" />
  </>
);
...
const FloatingActionButton = ({ onClick, icon, label, isLoading = false, disabled = false, isDarkMode = false, position = "bottom-6 right-6", className = "", }) => {
  ...
  return (
    <div className={["fixed z-40 group", position, className].filter(Boolean).join(" ")} >
      {/* Tooltip */}
      <div role="tooltip" className={[ "absolute bottom-[calc(100%+10px)] left-1/2 -translate-x-1/2", "px-3 py-1.5 rounded-lg border-2 text-xs font-black uppercase tracking-widest whitespace-nowrap", "pointer-events-none select-none", "opacity-0 group-hover:opacity-100 -translate-y-1 group-hover:translate-y-0", "transition-all duration-150", tooltipClasses, ].join(" ")} >
        {label} {isDarkMode ? <ArrowDark /> : <ArrowLight />}
      </div>
