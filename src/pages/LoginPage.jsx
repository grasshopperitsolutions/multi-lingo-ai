...
// Matches what api/auth.ts actually implements today — used until the real
// Firestore-backed state loads, and as the fallback if that fetch fails.
const DEFAULT_PROVIDERS_STATE = { google: true, apple: false, facebook: false, twitter: false };

const LoginPage = () => {
  const { isDarkMode, loginGoogle, loginApple, loginFacebook, loginTwitter, showAlert } = useAppContext();
  const { t } = useTranslation();
  const navigate = useNavigate();
  const location = useLocation();
  // Set by HomePage's pricing teaser when a visitor picks a paid plan while
  // logged out — resumed straight into Stripe Checkout after a successful login.
  const pendingPlan = location.state?.plan;
  const pendingInterval = location.state?.interval ?? "monthly";
  const [loadingProvider, setLoadingProvider] = useState(null);
  const [providers, setProviders] = useState(DEFAULT_PROVIDERS_STATE);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const result = await getAuthProviders(token);
        if (!cancelled) setProviders(result);
      } catch {
        // Keep DEFAULT_PROVIDERS_STATE — the login page must never hard-fail here.
      }
    })();
    return () => {
      cancelled = true;
        ...
        >
          ...
          <div className="flex flex-row justify-center gap-4">
            ...
            {(() => {
              ...
              return providers.google ? button : (
                <TooltipButton tooltip={t('login.provider_unavailable')} isDarkMode={isDarkMode}>
                  {button}
                </TooltipButton>
              );
            })()}
            ...
            {(() => {
              ...
              return providers.facebook ? button : (
                <TooltipButton tooltip={t('login.provider_unavailable')} isDarkMode={isDarkMode}>
                  {button}
                </TooltipButton>
              );
            ...
            {(() => {
              ...
              return providers.twitter ? button : (
                <TooltipButton tooltip={t('login.provider_unavailable')} isDarkMode={isDarkMode}>
                  {button}
                </TooltipButton>
              );
